...
import { getAllUseCaseSlugs, getUseCase } from "@/lib/use-cases/index";
...
export async function generateMetadata({
  ...
  return {
    title: `${title} | Olly Social - AI-Powered Social Media Assistant`,
    description,
    ...
    openGraph: {
      title: `${title} - Olly Social`,
      description,
      url: `https://www.olly.social/use-cases/${slug}`,
    ...
    twitter: {
      ...
      title: `${title} - AI-Powered Social Media`,
      description,
...
export default async function UseCasePage({ params }: UseCasePageProps) {
  ...
  const features = rawUseCase.features.map((feature) => {
    if (!validateFeatureIcon(feature.icon)) {
      console.warn(
        `Invalid icon type found: ${feature.icon}, defaulting to SearchIcon`,
      );
