...
export default function AdminBlogPage() {
  ...
  useEffect(() => {
    ...
    if (filteredPosts.length > 0 && selectedPosts.length === filteredPosts.length) {
      setSelectAll(true);
    } else {
      setSelectAll(false);
    }
  ...
  const groupPostsBySlug = (posts: BlogPost[]): GroupedBlogPost[] => {
    ...
    // Convert map to array of grouped posts
    return Array.from(groupedMap.entries()).map(([slug, posts]) => {
      ...
      return {
        ...
        isPublished: mainPost.isPublished,
        mainPost
      };
    });
  };
        ...
        <Card className="border-0 shadow-sm">
          <CardHeader className="bg-gradient-to-r from-slate-50 to-slate-100 rounded-t-lg">
            <div className="flex justify-between items-center">
              <CardTitle className="text-3xl font-bold text-slate-800">Blog Posts</CardTitle>
              <div className="flex gap-2">
                ...
                <Button 
                  onClick={() => router.push("/blog/admin/blog/new")}
                ...
                <table className="min-w-full bg-white">
                  <thead>
                    <tr className="bg-slate-50 text-slate-600 text-sm">
                      ...
                      <th className="py-3 px-4 border-b border-slate-200 text-left">Date</th>
                      <th className="py-3 px-4 border-b border-slate-200 text-left">Languages</th>
                      <th className="py-3 px-4 border-b border-slate-200 text-left">Categories</th>
                      <th className="py-3 px-4 border-b border-slate-200 text-center">Published</th>
                      <th className="py-3 px-4 border-b border-slate-200 text-center">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {groupedPosts.map((group) => (
                      <tr key={group.slug} className="hover:bg-slate-50 border-b border-slate-100 last:border-b-0">
                        <td className="py-3 px-4">
                          <div className="flex items-center">
                            <Checkbox 
                              checked={group.posts.every(post => selectedPosts.includes(post.id))}
                              onCheckedChange={(checked) => {
                                if (checked) {
                                  // Add all posts in this group to selected
                                  setSelectedPosts(prev => [
                                    ...prev, 
                                    ...group.posts.map(p => p.id).filter(id => !prev.includes(id))
                                  ]);
                                } else {
                                  // Remove all posts in this group from selected
                                  setSelectedPosts(prev => 
                                    prev.filter(id => !group.posts.some(p => p.id === id))
                                  );
                                }
                              }}
                              className="mr-2"
                            ...
                            <div>
                              ...
                              <div className="text-sm text-slate-500">{group.slug}</div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="py-3 px-4 text-center">
                          <Switch
                            ...
                            onCheckedChange={() => 
                              handleTogglePublish(group.mainPost.id, group.isPublished)
                            }
                          />
                        </td>
                        <td className="py-3 px-4">
                          <div className="flex justify-center gap-2">
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => router.push(`/blog/admin/blog/edit/${group.mainPost.id}`)}
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() => {
                                if (confirm(`Are you sure you want to delete all ${group.languages.length} language versions of this post?`)) {
                                  // Delete all posts in this group
                                  Promise.all(group.posts.map(post => 
