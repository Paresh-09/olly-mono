...
async function getPost(slug: string): Promise<{ metadata: BlogPostMetadata | null }> {
    try {
        ...
        if (!fs.existsSync(filePath)) {
            ...
            const langDirs = fs.readdirSync(path.join(process.cwd(), 'app/blog/_posts'))
                .filter(dir => dir !== '(en)' && !dir.includes('.'));
            
            for (const langDir of langDirs) {
                const langFilePath = path.join(process.cwd(), 'app/blog/_posts', langDir, `${slug}.mdx`);
...
export async function generateStaticParams(): Promise<Array<Params>> {
    const postsDirectory = path.join(process.cwd(), 'app/blog/_posts/(en)');
    const mdxPosts = fs.readdirSync(postsDirectory)
