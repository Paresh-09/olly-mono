...
export default function NewBlogPostPage() {
  ...
  const [formData, setFormData] = useState<BlogPostFormData>({
    ...
    isPublished: false,
    image: "",
    languages: AVAILABLE_LANGUAGES.reduce((acc, lang) => {
      acc[lang.code] = {
        content: "",
        description: "",
        title: "",
  ...
  const [generationProgress, setGenerationProgress] = useState<Record<string, 'pending' | 'success' | 'error'>>({});
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const blogPosts = Object.entries(formData.languages)
        .filter(([_, langData]) => langData.isEnabled && langData.content.trim())
  ...
  const handleSelectAllLanguages = (select: boolean) => {
    setFormData((prev) => ({
      ...prev,
      languages: Object.keys(prev.languages).reduce((acc, langCode) => {
        acc[langCode] = {
          ...prev.languages[langCode],
  ...
  const generateForLanguage = async (langCode: string): Promise<void> => {
    try {
      ...
      setError(`Failed to generate content for ${AVAILABLE_LANGUAGES.find(l => l.code === langCode)?.name}`);
  ...
  const generateWithAIForLanguages = async (languages: string[]) => {
    ...
    setGenerationProgress(languages.reduce((acc, lang) => {
      acc[lang] = 'pending';
      return acc;
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "An error occurred while generating content"
      );
      console.error(err);
    } finally {
          ...
          <CardContent className="p-6">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-2">
              <TabsList className="mb-6 w-full bg-slate-100 p-1">
                <TabsTrigger value="edit" className="flex-1 data-[state=active]:bg-white data-[state=active]:shadow-sm">Edit Post</TabsTrigger>
                <TabsTrigger value="preview" className="flex-1 data-[state=active]:bg-white data-[state=active]:shadow-sm">Preview</TabsTrigger>
              </TabsList>
                        ...
                        <div className="space-y-4">
                          <div>
                            ...
                            <p className="text-xs text-slate-500 mt-1">
                              The same slug will be used for all language versions. This helps group related posts together.
                            </p>
                          </div>

                          <div>
                            <Label htmlFor="date" className="text-slate-700 flex items-center gap-1">
                              <Calendar size={14} />
                              Date
                            </Label>
                            <Input
                          ...
                          <div>
                            <Label htmlFor="image" className="text-slate-700 flex items-center gap-1">
                              <Image size={14} />
                              Featured Image URL
                            </Label>
                            <Input
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2 mt-3">
                            ...
                            {formData.categories.length === 0 && (
                              <p className="text-xs text-slate-500 italic">No categories added</p>
                            )}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-slate-50 p-5 rounded-lg border border-slate-100">
                        <div className="flex justify-between items-center mb-4">
                          ...
                          <Button
                            ...
                            variant="outline"
                            size="sm"
                            onClick={() => handleSelectAllLanguages(enabledLanguagesCount < AVAILABLE_LANGUAGES.length - 1)}
                            className="text-xs"
                          >
                    ...
                    <div className="md:col-span-2">
                      <div className="border border-slate-200 rounded-lg overflow-hidden">
                        ...
                        >
                              ...
                              <Dialog open={aiDialogOpen} onOpenChange={setAiDialogOpen}>
                                ...
                                <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
                                  <DialogHeader className="sticky top-0 bg-white pb-4 border-b">
                                    ...
                                    <DialogDescription>
                                      Generate blog posts using AI. Provide a title and optional context.
                                    </DialogDescription>
                                  </DialogHeader>
                                  <div className="space-y-4 py-4">
                                    <div className="space-y-2">
                                      <Label htmlFor="ai-title">Blog Title</Label>
                                      <Input
                                        id="ai-title"
                                        placeholder="Enter a title for your blog post"
                                        value={aiGenerationInput.title}
                                        onChange={(e) => setAiGenerationInput(prev => ({ ...prev, title: e.target.value }))}
                                      />
                                    </div>
                                    <div className="space-y-2">
                                      ...
                                      <Textarea
                                        ...
                                        onChange={(e) => setAiGenerationInput(prev => ({ ...prev, extraContext: e.target.value }))}
                                      />
                                    </div>
                                    {Object.keys(generationProgress).length > 0 && (
                                      <div className="space-y-2 border rounded-lg p-4 bg-slate-50">
                                        <Label className="mb-2 block">Generation Progress:</Label>
                                        <div className="grid grid-cols-2 gap-2">
                                          {Object.entries(generationProgress).map(([langCode, status]) => (
                                            <div
                                              key={langCode}
                                              className={cn(
                          ...
                          {AVAILABLE_LANGUAGES.map(lang => (
                            <TabsContent key={lang.code} value={lang.code} className="p-4 bg-white">
                              <div className="space-y-4">
                                <div>
                                  <Label htmlFor={`title-${lang.code}`} className="text-slate-700">
                                    Title ({lang.name})
                                  </Label>
                                  <Input
                                    id={`title-${lang.code}`}
                                    value={formData.languages[lang.code].title}
                                    onChange={(e) => handleLanguageContentChange(e, lang.code, "title")}
                                    required={formData.languages[lang.code].isEnabled}
                                    className="mt-1 border-slate-200 focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                                ...
                                <div>
                                  <Label htmlFor={`description-${lang.code}`} className="text-slate-700">
                                    Description ({lang.name})
                                  </Label>
                                  <Textarea
                                    id={`description-${lang.code}`}
                                    value={formData.languages[lang.code].description}
                                    onChange={(e) => handleLanguageContentChange(e, lang.code, "description")}
                                    required={formData.languages[lang.code].isEnabled}
                                    rows={3}
                                    className="mt-1 border-slate-200 focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                                ...
                                <div>
                                  <Label htmlFor={`content-${lang.code}`} className="text-slate-700">
                                    Content ({lang.name}) - Markdown
                                  </Label>
                                  <Textarea
                                    id={`content-${lang.code}`}
                                    value={formData.languages[lang.code].content}
                                    onChange={(e) => handleLanguageContentChange(e, lang.code, "content")}
                                    required={formData.languages[lang.code].isEnabled}
                                    rows={20}
                                    className="mt-1 font-mono border-slate-200 focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                                    disabled={!formData.languages[lang.code].isEnabled}
...
Write your content in ${lang.name} using Markdown...`}
                                  />
                                  <p className="text-xs text-slate-500 mt-1">
                                    You can include a metadata block at the top of your content, but title and description must be entered manually above.
                                  </p>
                                </div>
                              </div>
                            </TabsContent>
                  ...
                  <div className="flex justify-end space-x-4 mt-8 pt-4 border-t border-slate-200">
                    ...
                    >
                      <Save size={16} />
                      {loading ? "Creating..." : `Create Posts (${enabledLanguagesCount})`}
                    </Button>
                  </div>
                </form>
              </TabsContent>
