...
export default function KeywordModal({
  ...
  // Get the actual max values - if editing, add the current values to available limits
  const effectiveMaxLikes =
    isEditing && initialKeyword ? maxLikes + initialKeyword.numLikes : maxLikes;
  ...
  useEffect(() => {
    if (open && initialKeyword) {
      ...
      setKeyword("#"); // Start with hashtag
      setNumLikes(Math.min(5, maxLikes));
      setNumComments(Math.min(3, maxComments));
      setHasChanges(false);
    }
  }, [open, initialKeyword, maxLikes, maxComments]);
  ...
  const handleKeywordChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Ensure hashtag is always present
    if (!value.startsWith("#")) {
      setKeyword("#" + value.replace(/^#+/, "")); // Remove any existing hashtags and add one
    } else {
      setKeyword(value);
    }
  };
  ...
  const handleNumLikesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value > effectiveMaxLikes) {
      // toast.error(
      //   `You can only allocate ${effectiveMaxLikes} likes (out of ${MAX_LIKES} total)`,
      // );
      toast({
  ...
  const handleSave = () => {
    ...
    if (trimmedKeyword.includes(" ")) {
      // toast.error("Only single words are allowed. Please enter one word or hashtag.");
      toast({
        title:
    ...
    //   // toast.error("Keyword must start with a hashtag (#).");
    //   toast({
    //     title: "Keyword must start with a hashtag (#).",
    //     variant: "destructive",
    //   });
    //   return;
    // }
    ...
    if (numComments > effectiveMaxComments) {
      // toast.error(
      //   `You can only allocate ${effectiveMaxComments} comments (out of ${MAX_COMMENTS} total)`,
      // );
      toast({
    ...
    // toast.success(`${isEditing ? "Updated" : "Added"} keyword "${keyword}"`);
    toast({
    ...
    <Dialog open={open} onOpenChange={handleCloseModal}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader className="flex flex-row items-center justify-between">
          <DialogTitle>{isEditing ? "Edit" : "Add"} Keyword Target</DialogTitle>
          ...
          <div className="space-y-2">
            ...
            {effectiveMaxLikes === 0 ? (
              <div className="bg-red-50 text-red-600 text-xs p-2 rounded-md flex items-center">
                ...
                No likes available. Reduce likes from feed or other keywords.
              </div>
            ) : (
