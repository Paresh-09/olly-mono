...
export default function LookByHashtagPage() {
  ...
  const searchHashtags = async () => {
    ...
    try {
      const response = await fetch(
        `/api/ac/linkedin/hashtags/search?q=${encodeURIComponent(hashtag.trim().replace('#', ''))}`)
  ...
  const handleHashtagClick = (tag: string) => {
    router.push(`/dashboard/auto-commenter/linkedin/hashtags/${encodeURIComponent(tag.replace('#', ''))}`)
  }
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            ) : hashtag.trim() && !isSearching ? (
              // No results state (only show after search)
